...
/**
 * CompressionStream is missing from older Safari/WebKit builds.  Compression
 * is only an optimisation for the optical transfer, so a sender without the
 * API must still be able to transmit the original bytes instead of failing
 * before the first QR frame is rendered.
 */
...
export async function packFile(
  ...
  const tryGzip = bytes.length >= 768 && !isPrecompressedType(type) && canGzip();
...
export function frameVerdictMessage(verdict: FrameVerdict): string | null {
  switch (verdict.kind) {
    case "older-sender":
      return `That screen is sending an older LumaDrop format (v${verdict.version}). Update the sending device.`;
    case "newer-sender":
      return `That screen is sending a newer LumaDrop format (v${verdict.version}). Update this app to receive it.`;
    case "unsupported-flags":
      return "That stream uses a LumaDrop feature this version cannot read. Update this app to receive it.";
    default:
      return null;
  }
}
