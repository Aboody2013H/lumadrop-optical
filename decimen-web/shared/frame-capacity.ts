...
/**
 * The selected frame size is a ceiling, not a reason to pad a one-block file
 * with thousands of zero bytes.  Keeping a small payload small lowers its QR
 * version substantially, which makes short text files much easier for a
 * camera to resolve and avoids a stream that appears almost frozen.
 */
export function effectiveFrameBytes(payloadBytes: number, selectedFrameBytes: number): number {
  return Math.min(selectedFrameBytes, payloadBytes + HEADER_LEN);
}
