...
test("small one-block payloads do not get padded to a dense maximum-size QR", () => {
  assert.equal(effectiveFrameBytes(100, 2953), 100 + HEADER_LEN);
  assert.equal(blockLength(effectiveFrameBytes(100, 2953)), 100);
});

test("multi-block payloads keep the selected frame size", () => {
  assert.equal(effectiveFrameBytes(10_000, 2953), 2953);
});
