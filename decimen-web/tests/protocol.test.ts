...
test("older WebKit without CompressionStream still sends arbitrary files", async () => {
  ...
  try {
    const source = new TextEncoder().encode("works without browser gzip\n".repeat(2_000));
    const packed = await packFile("notes.txt", "text/plain", source);
    const recovered = await unpackFile(packed.container);
...
test("a v3 receiver names a pre-versioning sender instead of going quiet", () => {
  ...
  for (const [marker, version] of [
    ...
    assert.match(frameVerdictMessage(verdict)!, /older LumaDrop format/);
  }
});
...
test("a v3 receiver names a newer sender instead of going quiet", () => {
  ...
  assert.match(frameVerdictMessage(verdict)!, /newer LumaDrop format/);
