...
import {
  ...
  pinchCameraZoom,
  wheelCameraZoom,
} from "../shared/camera-zoom";
import {
  normalizeCameraFacing,
  oppositeCameraFacing,
  type CameraFacing,
} from "../shared/camera-facing";

// Optional bridge supplied by the offline Android wrapper. Browser/PWA builds
// keep using the ordinary blob download link; the APK streams verified bytes
// through Android's Storage Access Framework in bounded chunks.
interface LumaNativeBridge {
  requestSave(name: string, mediaType: string): void;
  appendBase64(chunk: string): boolean;
  finishSave(): void;
}

declare global {
  interface Window {
    LumaDropNative?: LumaNativeBridge;
    __lumaBeginNativeWrite?: () => void;
    __lumaCancelNativeWrite?: () => void;
  }
}

let pendingNativeSave: OpticalFile | null = null;
...
window.__lumaBeginNativeWrite = () => {
  const file = pendingNativeSave;
  const bridge = window.LumaDropNative;
  if (!file || !bridge) return;
  const chunkSize = 48 * 1024;
  for (let offset = 0; offset < file.bytes.length; offset += chunkSize) {
    if (!bridge.appendBase64(base64Chunk(file.bytes.subarray(offset, offset + chunkSize)))) break;
  }
  bridge.finishSave();
  pendingNativeSave = null;
};

window.__lumaCancelNativeWrite = () => { pendingNativeSave = null; };
...
const zoomValue = document.getElementById("zoom-value") as HTMLOutputElement;
const zoomReset = document.getElementById("zoom-reset") as HTMLButtonElement;
const cameraSwitch = document.getElementById("camera-switch") as HTMLButtonElement;
...
let preferredFacing: CameraFacing = "environment";
...
/**
 * Digital zoom deliberately feeds the same crop to the preview and decoder.
 * Any tracking boxes were measured in the old coordinate space, so changing
 * zoom drops only those boxes and forces a fresh full-frame acquisition; the
 * fountain decoder and all blocks already received remain intact.
 */
function setCameraZoom(value: number): void {
  const next = clampCameraZoom(value);
  if (next === cameraZoom) return;
  cameraZoom = next;
  cfgZoom.value = String(next);
  zoomValue.value = zoomLabel(next);
  zoomReset.textContent = zoomLabel(next);
  video.style.transform = next === 1 ? "" : `scale(${next})`;
  regions.length = 0;
  expectedRegions = 0;
  expectedRegionsAt = performance.now();
  lastFullScan = 0;
}
...
cfgZoom.addEventListener("input", () => setCameraZoom(Number(cfgZoom.value)));
zoomReset.addEventListener("click", () => setCameraZoom(1));
cameraBox.addEventListener(
  "wheel",
  (event) => {
    event.preventDefault();
    setCameraZoom(wheelCameraZoom(cameraZoom, event.deltaY));
  },
  { passive: false },
);
cameraBox.addEventListener(
...
cameraBox.addEventListener(
  ...
  (event) => {
    ...
    event.preventDefault();
    setCameraZoom(pinchCameraZoom(pinchStartZoom, pinchStartDistance, touchDistance(event.touches)));
  },
  { passive: false },
);
cameraBox.addEventListener("touchend", (event) => {
...
function updateCameraSwitchButton(): void {
  const reported = stream?.getVideoTracks()[0]?.getSettings().facingMode;
  if (reported === "user" || reported === "environment") {
    preferredFacing = normalizeCameraFacing(reported);
  }
  const label = preferredFacing === "user" ? msg.receive.switchToRear : msg.receive.switchToFront;
  cameraSwitch.textContent = label;
  cameraSwitch.setAttribute("aria-label", label);
}
...
async function populateCameraOptions() {
  ...
  cameraSwitch.hidden = cameras.length < 2;
...
async function start() {
  ...
  if (!settingsWired) {
    ...
    cameraSwitch.addEventListener("click", () => void toggleFacingCamera());
  }
 ...
 */
async function switchCamera(selection: MediaTrackConstraints = cameraSelection()) {
  if (done || !stream) return false;
  cfgCamera.disabled = true; // one switch at a time
  const previous = stream.getVideoTracks()[0]?.getSettings().deviceId;
  for (const track of stream.getTracks()) track.stop();
  let refused = false;
  try {
    stream = await acquireCamera(selection);
  } catch {
    try {
...
async function toggleFacingCamera() {
  if (done || !stream || cameraSwitch.disabled) return;
  const previousFacing = preferredFacing;
  const target = oppositeCameraFacing(preferredFacing);
  preferredFacing = target;
  cfgCamera.value = "";
  cameraSwitch.disabled = true;
  const switched = await switchCamera({ facingMode: { exact: target } });
  if (!switched) {
    preferredFacing = previousFacing;
    updateCameraSwitchButton();
  }
...
function captureFrame() {
  ...
  if (BITMAP_CAPTURE && cameraZoom === 1) {
    if (fullScanDue) {
      lastFullScan = now;
      fullScans++;
  ...
  const crop = cameraZoomCrop(vw, vh, cameraZoom);
  ctx.drawImage(video, crop.x, crop.y, crop.width, crop.height, 0, 0, vw, vh);
  if (fullScanDue) {
    lastFullScan = now;
    fullScans++;
...
async function finish(container: Uint8Array, hashOk: boolean, seconds: number) {
  ...
  try {
    ...
    if (window.LumaDropNative) {
      download.addEventListener("click", (event) => {
        event.preventDefault();
        pendingNativeSave = file;
        window.LumaDropNative!.requestSave(file.name, file.type);
      });
    }
