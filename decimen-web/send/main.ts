...
import { storeZip } from "../shared/zip-store";
...
interface SenderNativeBridge {
  sharedFilesJson?(): string;
  clearSharedFiles?(): void;
  setTransferActive?(active: boolean): void;
}

const nativeBridge = (window as unknown as { LumaDropNative?: SenderNativeBridge }).LumaDropNative;

function setNativeTransferActive(active: boolean): void {
  nativeBridge?.setTransferActive?.(active);
}
...
function updateFilePicker(): void {
  ...
  filePickerLabel.textContent =
    armed && selectedFile
      ? msg.send.selectedFile(selectedFile.displayName)
...
function applyMode(): void {
  ...
  if (mode === "file" && cfgFile.files?.length) void selectFiles([...cfgFile.files]);
}
...
async function startSelection(
  status: string,
  prepare: () => Promise<{ name: string; displayName?: string; size: number; packed: PackedOpticalFile }>,
): Promise<void> {
  setNativeTransferActive(false);
  ...
  setStatus(status);
  try {
    const { name, displayName = name, size, packed } = await prepare();
...
async function selectFiles(files: readonly File[]): Promise<void> {
  ...
  const selectionName = files.length === 1 ? files[0]!.name : `${files.length} files`;
  await startSelection(msg.send.preparingFile(selectionName), async () => {
    ...
    if (empty) {
      throw new Error(msg.send.fileEmpty(empty.name));
    }
    if (totalSize > MAX_FILE_BYTES) {
      throw new Error(msg.send.fileOverLimit(selectionName, formatBytesL(totalSize), MAX_FILE_LABEL));
    }
    if (files.length === 1) {
      const file = files[0]!;
      ...
      return {
        name: file.name,
        size: file.size,
        packed: await packFile(file.name, file.type, bytes),
      };
    }
    const entries = [];
    ...
    const name = `lumadrop-${files.length}-files.zip`;
    return {
      name,
      displayName: `${files.length} files · ${formatBytesL(totalSize)}`,
      size: totalSize,
      packed: await packFile(name, "application/zip", zip),
    };
  });
  updateFilePicker();
}

async function importNativeShare(): Promise<void> {
  const json = nativeBridge?.sharedFilesJson?.();
  if (!json) return;
  let manifest: Array<{ url: string; name: string; type: string }>;
  try {
    manifest = JSON.parse(json) as Array<{ url: string; name: string; type: string }>;
  } catch {
    return;
  }
  ...
  try {
    ...
    for (const item of manifest) {
      ...
      if (!response.ok) throw new Error(`could not open ${item.name}`);
      files.push(new File([await response.blob()], item.name, { type: item.type }));
    }
    await selectFiles(files);
  } catch (error) {
    showError(localizeError(error));
  } finally {
    nativeBridge?.clearSharedFiles?.();
  }
}
...
async function main() {
  ...
  applyMode();
  if (!DEMO && !BENCHMARK) await importNativeShare();
...
async function startStream(revealStage = false) {
  ...
  const actualFrameBytes = effectiveFrameBytes(payload.length, frameBytes);
  const blockLen = blockLength(actualFrameBytes);
  ...
  const makeCell = (): ImageData => {
    ...
    if (version === undefined) {
      ...
      spec("spec-frame").textContent = msg.send.frameBytesValue(String(actualFrameBytes), gridCodes);
  ...
  pump();
  if (!generatorFailed) setNativeTransferActive(true);
...
window.addEventListener("pagehide", () => setNativeTransferActive(false));
